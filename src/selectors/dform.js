...
export const getDformStyles = (state: Object): {[key: string]: List<DformStyle>} => state.dform.styles;

export const getRecordFormStyles = (state: Object): List<Map<string, any>> => state.dform.styles.get('record', Immutable.List([]));
...
export const getDefaultRecordFormStyleId = (state: Object): ?number => getViewableRecordFormStyles(state).getIn([0, 'id']);

export const getRecordFormStyleId = (state: Object): ?number => state.patientView.recordFormStyleId || getDefaultRecordFormStyleId(state);
...
export const getPatientFormStyle = (state: Object): List<Map<string, any>> => state.dform.styles.getIn(['patient', 0, 'style']); // TODO とりあえず固定で
